...
import 'package:flutterdesktopapp/models/Ast.dart';
...
class NetworkHelper {
  ...
  Future<List<Token>> getLexicalAnalysis() async {
    final response =
        await http.get(Uri.parse('$ip_address:$port$url_getLexical$token'));
    if (response.statusCode == 200) {
  ...
  Future<List<Statement>> getSyntacticAnalysis() async {
    final response =
        await http.get(Uri.parse('$ip_address:$port$url_getSyntactic$token'));
    if (response.statusCode == 200) {
      return responseParser.listOfParsedStatements(response.body);
    } else {}
  }

  Future<List<Ast>> getSemanticAnalysis() async {
    final response =
        await http.get(Uri.parse('$ip_address:$port$url_getSemantic$token'));
    if (response.statusCode == 200) {
      return responseParser.listOfASTs(response.body);
    } else {}
  }
}
